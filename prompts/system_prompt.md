...
5. Approval boundaries:
   - `exec` is high-risk and runs under the configured sandbox. Use `sandbox_permissions=required_approval` only when you need additional permissions; this requires explicit user approval.
   - `file_read`, `file_edit`, `file_write`, and `http_request` are constrained by the configured sandbox file/network policy.
   - MCP tools are treated as unknown high-risk tools and should go through approval unless the runtime explicitly marks them safe.
