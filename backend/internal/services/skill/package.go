...
func (s *SkillPackageService) InstallFromZip(filename string, data []byte) (*domain.Skill, error) {
	...
	if len(data) > constants.MaxSkillZipBytes {
		return nil, fmt.Errorf("zip file is too large, maximum allowed is %d MB", constants.MaxSkillZipBytes/(1024*1024))
	}
	...
	exists, err := s.store.GetSkillByName(meta.Name)
	if err != nil {
		return nil, err
	}
	if exists != nil {
	...
	item, err := s.store.CreateSkill(domain.Skill{
		Name:         meta.Name,
...
func (s *SkillPackageService) validateZipAndCollect(data []byte) (*parsedSkillMetadata, map[string]*zip.File, error) {
	...
	if len(reader.File) > constants.MaxSkillFileCount {
		return nil, nil, fmt.Errorf("too many files in zip, maximum allowed is %d", constants.MaxSkillFileCount)
	}
	...
	for _, file := range reader.File {
		...
		if !file.FileInfo().IsDir() {
			if file.UncompressedSize64 > constants.MaxSkillSingleFileSize {
				return nil, nil, fmt.Errorf("file %s exceeds single-file size limit %d MB", cleanName, constants.MaxSkillSingleFileSize/(1024*1024))
			}
	...
	if total > constants.MaxSkillExtractedBytes {
		return nil, nil, fmt.Errorf("total extracted size exceeds limit %d MB", constants.MaxSkillExtractedBytes/(1024*1024))
	}
...
func readSkillMetadata(file *zip.File, expectedName string) (*parsedSkillMetadata, error) {
	...
	raw, err := io.ReadAll(io.LimitReader(rc, constants.MaxSkillSingleFileSize))
	if err != nil {
		return nil, fmt.Errorf("failed to read SKILL.md: %w", err)
	}
...
func listSkillResourceFiles(skillDir string) ([]string, error) {
	...
	err := filepath.WalkDir(skillDir, func(p string, d fs.DirEntry, walkErr error) error {
		...
		if len(results) >= constants.MaxSkillResourcesShown {
			return fs.SkipAll
		}
		return nil
	})
