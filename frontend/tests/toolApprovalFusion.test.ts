...
test('ToolCallCard renders subagent thinking entries with the shared ThinkingBlock', () => {
  ...
  assert.match(toolCallCardSource, /subagentTimelineRows/)
  assert.match(toolCallCardSource, /buildLightweightToolRows/)
  ...
  assert.match(toolCallCardSource, /v-for="timelineRow in subagentTimelineRows"/)
...
test('AssistantMessageBody renders the parent reply collapse bar and visible timeline', () => {
  ...
  assert.match(assistantBodySource, /buildLightweightToolTimelineRows/)
  assert.match(assistantBodySource, /v-for="\((row, index|row,\s*index)\) in renderedRows"/)
