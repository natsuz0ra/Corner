...
<template>
  <div class="h-screen bg-gray-50 flex items-center justify-center p-2 sm:p-3">
    <!-- 外层卡片容器 -->
    <div class="relative w-full h-full max-w-[1200px] bg-white rounded-xl border border-gray-200 flex overflow-hidden shadow-sm">
      ...
      <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="relative flex items-center justify-center h-12 border-b border-gray-100 bg-white flex-shrink-0">
          ...
          >
            <span class="text-sm font-medium truncate">{{ currentSession?.name || t('newSession') }}</span>
        ...
        >
          <!-- 加载中 -->
          <div v-if="loading" class="flex items-center justify-center h-full text-sm text-gray-400">
          ...
          <div class="flex flex-col gap-6 max-w-[688px] mx-auto">
            <div
              ...
              :class="[
                ...
                item.role === 'user' && store.isFailedUserMessage(item.id) ? 'items-end' : 'items-start',
                item.role === 'assistant' && isEmptyPlaceholder(item.id) && store.waiting ? 'items-center' : '',
              ]"
            >
              <!-- AI 头像 -->
              <div
        ...
        <footer class="flex-shrink-0 px-4 py-3 bg-white border-t border-gray-100">
          <div class="max-w-[620px] mx-auto">
            <div class="relative border border-gray-200 rounded-2xl bg-white focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all duration-200 overflow-hidden">
              <textarea
                v-model="inputValue"
                class="w-full resize-none border-0 outline-none bg-transparent px-4 pt-3 pb-10 text-sm text-gray-800 placeholder-gray-400 leading-relaxed min-h-[44px] max-h-[200px]"
              ...
              <div class="absolute bottom-2 left-3 right-3 flex items-center justify-between">
                ...
                <select
                  ...
                  class="text-xs text-gray-500 bg-transparent border-0 outline-none cursor-pointer hover:text-gray-700 transition-colors duration-150 max-w-[140px] truncate"
                  :disabled="modelOptions.length === 0"
                  @change="onModelChange(($event.target as HTMLSelectElement).value)"
                >
                  <option v-if="modelOptions.length === 0" value="">{{ t('newSession') }}</option>
